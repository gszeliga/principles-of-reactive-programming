...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: scala.concurrent.Future[T]) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      // This block does not work since is defined within a value object.
      // http://stackoverflow.com/questions/20252687/implementation-restriction-nested-class-is-not-allowed-in-value-class-this-res
      //      async {
      //
      //        var pending = fs
      //        val lst = ListBuffer[T]()
      //
      //        while (!pending.isEmpty) {
      //          lst += await { pending.head }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      val assignFirst: Try[T] => Unit = p complete _

      fs foreach { _ onComplete assignFirst }
